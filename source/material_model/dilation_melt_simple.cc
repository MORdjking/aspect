...
#include <aspect/material_model/dilation_melt_simple.h>
...
namespace aspect
{
  namespace MaterialModel
  {
    template <int dim>
    double
    DilationMeltSimple<dim>::
    reference_darcy_coefficient () const
    {
      // 0.01 = 1% melt
      return reference_permeability * std::pow(0.01,3.0) / eta_f;
      std::cout << "reference_darcy_coefficient = " << reference_permeability * std::pow(0.01,3.0) / eta_f << std::endl;
    }
    ...
    is_compressible () const
    {
      ...
      std::cout << "is_compressible = " << model_is_compressible << std::endl;
    }
                   ...
                   const double pressure) const
    {
      ...
      std::cout << "melt_fraction" << std::endl;
    }
                    ...
                    const NonlinearDependence::Dependence dependence) const
    {
      ...
      std::cout << std::endl;
      std::cout << "entropy_change" << std::endl;
    }
...
//        std::cout << std::endl;
//        std::cout << "melt_fractions = " << melt_fractions[q] << std::endl;
        }
      std::cout << std::endl;
      std::cout << "melt_fractions" << std::endl;
    }


    template <int dim>
    void
    DilationMeltSimple<dim>::initialize ()
    { 
      if (this->include_melt_transport())
        {
          AssertThrow(this->get_parameters().use_operator_splitting,
                      ExcMessage("The material model ``Dilation melt simple'' can only be used with operator splitting!"));
          AssertThrow(this->introspection().compositional_name_exists("peridotite"),
                      ExcMessage("Material model Dilation melt simple only works if there is a "
                                 "compositional field called peridotite."));
          AssertThrow(this->introspection().compositional_name_exists("porosity"),
                      ExcMessage("Material model Dilation melt simple with melt transport only "
                                 "works if there is a compositional field called porosity."));
        }
      std::cout << std::endl;
      std::cout << "(((((((((((INITIALIZE STEP)))))))))))" << std::endl;  
    }

    template <int dim>
    void
    DilationMeltSimple<dim>::update ()
    {
      std::cout << std::endl;
    ...
    template <int dim>
    void
    DilationMeltSimple<dim>::declare_parameters (ParameterHandler &prm)
    {
      prm.enter_subsection("Material model");
      {
        prm.enter_subsection("Dilation melt simple");
        {
    ...
    template <int dim>
    void
    DilationMeltSimple<dim>::parse_parameters (ParameterHandler &prm)
    {
      prm.enter_subsection("Material model");
      {
        prm.enter_subsection("Dilation melt simple");
        {
          ...
          if (thermal_viscosity_exponent!=0.0 && reference_T == 0.0)
            AssertThrow(false, ExcMessage("Error: Material model Dilation melt simple with Thermal viscosity exponent can not have reference_T=0."));
    ...
    template <int dim>
    void
    DilationMeltSimple<dim>::create_additional_named_outputs (MaterialModel::MaterialModelOutputs<dim> &out) const
    {
...
namespace aspect
{
  namespace MaterialModel
  {
    ASPECT_REGISTER_MATERIAL_MODEL(DilationMeltSimple,
                                   "dilation melt simple",
